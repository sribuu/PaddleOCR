...
class HyperParameters(object):
    ...
    def __init__(
        ...
        use_visualdl=False,
        seed=2022,
        infer_img="./train_data/kie/test.txt",
        infer_mode=False,
        ...
        algorithm="LayoutXLM",
        Transform=None,
        architecture_name="LayoutXLMForSer",
        pretrained=True,
        checkpoints=None,
        mode="vi",
        num_classes=105,
        loss_reduction="mean",
        loss_name="VQASerTokenLayoutLMLoss",
        key="backbone_out",
        ...
        self.config["Global"]["save_model_dir"] = os.path.join(model_dir,"model_paddle_checkpoint")
        ...
        self.config["Global"]["save_inference_dir"] = os.path.join(model_dir,"model_paddle")
        ...
        self.config["Global"]["save_res_path"] = os.path.join(model_dir,"output")
        ...
        if self.global_model in ["SER", "ALL"]:
            self.config["PostProcess"]["class_path"] = os.path.join(model_dir,postprocess_class_path) 
        ...
        if self.global_model in ["SER", "ALL"]:
            ...
            self.config["Train"] = {
                "dataset": {
                    "name": "SimpleDataSet",
                    "data_dir": os.path.join(model_dir,"dataset"),
                    "label_file_list": [
                        os.path.join(model_dir,"train_data/kie/train.txt")],
                    "ratio_list": [1.0],
                    "transforms": [
                        {"DecodeImage": {"img_mode": "RGB", "channel_first": False}},
                        {
                            "VQATokenLabelEncode": {
                                "contains_re": False,
                                "algorithm": algorithm,
                                "class_path": os.path.join(model_dir,"label-key-list.txt"),
            ...
            self.config["Eval"] = {
                "dataset": {
                    "name": "SimpleDataSet",
                    "data_dir":os.path.join(model_dir,"dataset"),
                    "label_file_list": [os.path.join(model_dir,"train_data/kie/val.txt")],
                    "transforms": [
                        {"DecodeImage": {"img_mode": "RGB", "channel_first": False}},
                        {
                            "VQATokenLabelEncode": {
            ...
            # Read the entities
            fn_key = os.path.join(model_dir,"label-key-list.tx")
            entities_labels = {}
            counter = 0
            ...
            self.config["Train"] = {
                "dataset": {
                    "name": "SimpleDataSet",
                    "data_dir":os.path.join(model_dir,"dataset"),
                    "label_file_list": [os.path.join(model_dir,"train_data/kie/train.txt")],
                    "ratio_list": [1.0],
                    "transforms": [
                        {"DecodeImage": {"img_mode": "RGB", "channel_first": False}},
                        {
                        ...
                        {"TensorizeEntitiesRelations": None},
                        {"Resize": {"size": [224, 224]}},
                        {
                            "NormalizeImage": {
                                "scale": 1,
...
if __name__ == "__main__":
    cwd = os.getcwd()
    c = HyperParameters(model_dir=cwd)
    d = c.load_config()
